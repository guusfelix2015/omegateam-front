...
export const raidDroppedItemsService = {
  ...
  async getAll(
    params: GetRaidDroppedItemsParams = {}
  ): Promise<RaidDroppedItemsList> {
    try {
      const searchParams = new URLSearchParams();

      if (params.page) searchParams.append('page', params.page.toString());
      if (params.limit) searchParams.append('limit', params.limit.toString());
      if (params.raidInstanceId)
        searchParams.append('raidInstanceId', params.raidInstanceId);
      ...
      const response = await api.get(
        `/raid-dropped-items?${searchParams.toString()}`
      );
  ...
  async getByRaidInstanceId(
    raidInstanceId: string
  ): Promise<RaidDroppedItem[]> {
    try {
      const response = await api.get(
        `/raid-instances/${raidInstanceId}/dropped-items`
      );
      // Backend returns { data: RaidDroppedItem[] }
      const data = response.data.data || response.data;
      return Array.isArray(data)
        ? data.map((item) => RaidDroppedItemSchema.parse(item))
        : [];
    } catch (error) {
