...
export default function RaidsDashboard() {
  ...
  const { data: instanceStats, isLoading: isLoadingInstanceStats } = useRaidInstanceStats();
  const { data: activeRaids, isLoading: isLoadingActiveRaids } = useActiveRaids();
  const { data: recentInstances, isLoading: isLoadingRecentInstances } = useRecentRaidInstances(5);

  const isLoading = isLoadingRaidStats || isLoadingInstanceStats || isLoadingActiveRaids || isLoadingRecentInstances;
      ...
      <div className="space-y-6">
          ...
          <Card>
            <CardContent className="pt-6">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm text-muted-foreground">Total de Raids</p>
            ...
            <CardContent className="pt-6">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-2xl font-bold text-green-600">{raidStats?.active || 0}</p>
                </div>
              </div>
            </CardContent>
          </Card>
            ...
            <CardContent className="pt-6">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-2xl font-bold text-blue-600">{instanceStats?.total || 0}</p>
                </div>
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            <CardContent className="pt-6">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm text-muted-foreground">DKP Distribuído</p>
                  <p className="text-2xl font-bold text-yellow-600">{instanceStats?.totalDkpAwarded || 0}</p>
                </div>
              </div>
            </CardContent>
          </Card>
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Raids disponíveis para execução
              </CardDescription>
            </CardHeader>
            <CardContent>
              {!activeRaids || activeRaids.length === 0 ? (
                ...
                <div className="space-y-3">
                  {activeRaids.slice(0, 5).map((raid) => (
                    <div key={raid.id} className="flex items-center justify-between p-3 bg-muted rounded-lg">
                      <div>
              ...
              {!recentInstances || recentInstances.length === 0 ? (
                ...
                <div className="space-y-3">
                  {recentInstances.map((instance) => (
                    <div key={instance.id} className="flex items-center justify-between p-3 bg-muted rounded-lg">
                      <div className="flex items-center gap-3">
        ...
        <div className="grid gap-6 md:grid-cols-3">
          <Card>
            ...
            <CardContent className="space-y-3">
              <div className="flex justify-between">
                <span className="text-muted-foreground">Nível Médio dos Bosses:</span>
                <span className="font-semibold">{Math.round(raidStats?.averageBossLevel || 0)}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-muted-foreground">Score Base Médio:</span>
                <span className="font-semibold">{Math.round(raidStats?.averageBaseScore || 0)}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-muted-foreground">Participantes Totais:</span>
                <span className="font-semibold">{instanceStats?.totalParticipants || 0}</span>
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent className="space-y-3">
              <div className="flex justify-between">
                <span className="text-muted-foreground">Média de Participantes:</span>
                <span className="font-semibold">
                  {Math.round(instanceStats?.averageParticipantsPerRaid || 0)}
                </span>
              </div>
              <div className="flex justify-between">
                <span className="text-muted-foreground">DKP Médio por Raid:</span>
                <span className="font-semibold">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-semibold">
                  {raidStats?.total ? Math.round((raidStats.active / raidStats.total) * 100) : 0}%
                </span>
              </div>
            </CardContent>
          </Card>
