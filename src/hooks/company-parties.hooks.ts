...
import { type CreateCompanyParty, type UpdateCompanyParty, type AddPlayerToParty } from '../types/api';
...
export const useCreateCompanyParty = () => {
  ...
  return useMutation({
    mutationFn: (data: CreateCompanyParty) => companyPartiesService.create(data),
    onSuccess: () => {
...
export const useAddPlayerToParty = () => {
  ...
  return useMutation({
    mutationFn: ({ partyId, data }: { partyId: string; data: AddPlayerToParty }) =>
      companyPartiesService.addPlayer(partyId, data),
    onSuccess: (_, { partyId }) => {
      queryClient.invalidateQueries({ queryKey: ['company-parties'] });
      queryClient.invalidateQueries({ queryKey: ['company-parties', partyId] });
      queryClient.invalidateQueries({ queryKey: ['company-parties', partyId, 'members'] });
      toast({
...
export const useRemovePlayerFromParty = () => {
  ...
  return useMutation({
    mutationFn: ({ partyId, playerId }: { partyId: string; playerId: string }) =>
